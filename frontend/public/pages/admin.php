...
/**
 * Autoriser si:
 *  - rôle brut DB = "Administrateur"
 *  - OU type normalisé en session = "admin"
 */
requireRole(['Administrateur', 'admin']);
...
try {
    ...
    // Revenus par jour (2 crédits par participant)
    $stmt = $pdo->query("
    ...
    $totalPlatformCredits = (int)($stmt->fetch()['total_credits'] ?? 0);

} catch (Throwable $e) {
    error_log('[ADMIN][ERR] ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
...
// Traitement des actions (suspension / activation / création employé)
if ($_POST && isset($_POST['action'])) {
    try {
        $pdo = getDatabase(); // <-- s'assurer que $pdo existe ici aussi
        ...
        if ($_POST['action'] === 'suspend_user') {
            ...
            if (in_array($role, ['Moderateur', 'Administrateur'], true)) {
...
<head>
    ...
    <!-- chemins relatifs pour local & Heroku -->
...
<main style="max-width:1200px;margin:40px auto;padding:0 20px;">
    <div style="background:white;padding:30px;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.1);">
            ...
            <table class="admin-table">
                ...
                <?php foreach ($users as $u): ?>
                    <tr>
                        ...
                        <td><span class="admin-badge <?= strtolower($u['status']) ?>"><?= htmlspecialchars(ucfirst($u['status'])) ?></span></td>
                        <td><?= (int)$u['credits'] ?></td>
                        <td>
                            <?php if ($u['status'] === 'actif'): ?>
                                <button onclick="suspendUser(<?= (int)$u['id'] ?>)" class="admin-table button">Suspendre</button>
                            <?php else: ?>
                                <button onclick="activateUser(<?= (int)$u['id'] ?>)" class="admin-table button">Réactiver</button>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
        ...
        <form method="POST" class="form-container">
            ...
            <input type="password" name="password" placeholder="Mot de passe temporaire (min 6)" required>
            <select name="role">
...
<script>
    ...
    const tripsData = <?= json_encode($tripsByDay, JSON_UNESCAPED_UNICODE) ?>;
    const revenueData = <?= json_encode($revenuesByDay, JSON_UNESCAPED_UNICODE) ?>;
    ...
    new Chart(tripsCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: tripsData.map(d => Number(d.count)),
                borderColor: '#00b894',
    ...
    new Chart(revenueCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: revenueData.map(d => Number(d.credits)),
